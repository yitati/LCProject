...
* relationship between two numbers. And our secret signature was constructed by a special integer 
* array, which contains uniquely all the different number from 1 to n (n is the length of the 
* secret signature plus 1). For example, the secret signature "DI" can be constructed by array [2,1,3] 
* or [3,1,2], but won't be constructed by array [3,2,4] or [2,1,3,4], which are both illegal constructing 
...
// whenever we saw 'D', which means nums[i] and nums[i+1] should be in decresing order
// then we shold swap them, but what if we have multiple 'D'? 
// we should reverse order between continueous 'D' s
